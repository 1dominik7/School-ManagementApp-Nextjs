...
export const createAssignment = async (
  ...
  try {
    if (role === "teacher") {
      const teacherAssignment = await prisma.lesson.findFirst({
        where: {
          teacherId: userId!,
          id: data.lessonId,
        },
...
export const createResult = async (
  ...
  try {
    if (role === "teacher") {
      const teacherResult = await prisma.lesson.findFirst({
        where: {
          teacherId: userId!,
          id: data.id,
        },
    ...
    if (data.assignmentId && data.assignmentId !== 0) {
...
export const deleteResult = async (
  ...
  try {
    await prisma.result.delete({
      where: {
        id: parseInt(id),
        ...(role === "teacher" ? { result: { teacherId: userId! } } : {}),
      },
    });
